...
def process_response(cur, conn, user_name, user_id, wt_msg, command):
    ...
    if(command!=None):
        ...
        if(command.lower() == 'book follow up'):
            send_to = 0
            response_type = 0
        ...
        if(command.lower() == 'word follow up'):
            send_to = 0
            response_type = 0
        ...
        if(command.lower() == 'article follow up'):
            send_to = 0
            response_type = 0
        ...
        if(command.lower() == 'wake up graph'):
            today_date = date.today()
            td = timedelta(28)
        ...
        if(space_sep[0].isnumeric()):
            ...
            if(len(pts)==0):
                ...
                if(activity_id>27 or activity_id<25):
                    if(remaining_part!=''):
            ...
            if(space_sep[1].lower()=='activity'):
                ...
                try:
                    ...
                    activity_name = ''
                    for inde in range(2,(len(space_sep)-1)):
                    ...
                    cur.execute('INSERT INTO LIST_OF_ACTIVITIES VALUES (?,?,?,?)',(activity_id,1,activity_name,points))
                    #conn.commit()

                    response_msg = user_name + ' inserted an activity "' + activity_name
                    response_msg += '" with ' + str(points) + ' points for one repetition. '
